...
boolean EL3104_InitDc(uint16 slave)
{
   ...
   //ec_dcsync01(slave, TRUE, 1000000, 200000, 400000);
   return FALSE;
}
...
void InitializeSlavesSoToOp(OSAL_EVENT_HANDLE * timerEvent)
{
   ...
   do
   {
      ...
      for (i = 0; i < j; i++)
      {
         ec_FPRD(slaveDcId[i] + SLAVE_NUMBER_TO_SLAVE_ADDRESS_PHYSIC, DC_DIFFERENCE_REGISTER_ADDRESS, DC_DIFFERENCE_REGISTER_LENGTH, &dcDifference, EC_TIMEOUTRET3);
         if (dcDifference > 4)
            break;
      }
   }
